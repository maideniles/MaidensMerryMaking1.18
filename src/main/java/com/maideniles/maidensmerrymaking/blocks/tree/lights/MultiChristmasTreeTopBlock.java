...
public class MultiChristmasTreeTopBlock extends BushBlock {

    protected static final VoxelShape SHAPE = Block.box(2.0D, 0.0D, 2.0D, 14.0D, 16.0D, 14.0D);

    public MultiChristmasTreeTopBlock(Properties properties) {
        super(properties);
    ...
    protected boolean mayPlaceOn(BlockState state, BlockGetter level, BlockPos pos) {
        ...
        return block == ModBlocks.CHRISTMAS_TREE_MULTI_LIGHTS_MIDDLE.get();
    }
    ...
    public void playerWillDestroy(Level level, BlockPos pos, BlockState state, Player player) {
        ...
        if ( blockstate2.getBlock() == ModBlocks.CHRISTMAS_TREE_MULTI_LIGHTS_MIDDLE.get()) {
    ...
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand handIn, BlockHitResult pHit) {
        ...
        if (!level.isClientSide()) {
            ...
            if (tree.getItem() == ModItems.ORNAMENTS.get()){
                ...
                level.setBlock(pos.below(2), ModBlocks.DECORATED_MULTI_CHRISTMAS_TREE_BOTTOM.get().defaultBlockState(),3);
                level.setBlock(pos.below(), ModBlocks.DECORATED_MULTI_CHRISTMAS_TREE_MIDDLE.get().defaultBlockState(),3);
                level.setBlock(pos, ModBlocks.DECORATED_MULTI_CHRISTMAS_TREE_TOP.get().defaultBlockState(),3);
        ...
        if (!level.isClientSide()) {
            ...
            if (tree.getItem() == ModItems.ORNAMENTS_WHITE.get()){
                ...
                System.out.println("O CHRISTMAS TREE, O CHRISTMAS TREE!");
                level.setBlock(pos.below(2), ModBlocks.DECORATED_WHITE_MULTI_CHRISTMAS_TREE_BOTTOM.get().defaultBlockState(),3);
                level.setBlock(pos.below(), ModBlocks.DECORATED_WHITE_MULTI_CHRISTMAS_TREE_MIDDLE.get().defaultBlockState(),3);
                level.setBlock(pos, ModBlocks.DECORATED_WHITE_MULTI_CHRISTMAS_TREE_TOP.get().defaultBlockState(),3);
